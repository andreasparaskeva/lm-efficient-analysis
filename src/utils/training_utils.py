...
def get_training_args(
    ...
    # Resolve effective per-device batch size and gradient accumulation
    ...
    # Compute effective global batch (for info & reproducibility)
    ...
    # Multi-GPU / Distributed toggle
    if num_gpus > 1:
        print(f"[TrainingArgs] Multi-GPU mode enabled ({num_gpus} GPUs)")
        try:
